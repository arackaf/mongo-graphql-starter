...
beforeAll(async () => {
  ...
  await db.collection("books").insert({ title: "Book 1", keywords: ["+ and ?"] });
  await db.collection("books").insert({ title: "Book 2", keywords: ["+ and ?", "?"] });
  await db.collection("books").insert({ title: "Book 3", keywords: ["? foo"] });
  await db.collection("books").insert({ title: "Book 4", keywords: ["ends +"] });
});
