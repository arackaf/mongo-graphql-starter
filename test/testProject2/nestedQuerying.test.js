...
test("Deep querying 2", async () => await runIt(`{allBlogs(author: {name_endsWith: "2"}, SORT: {title: 1}){Blogs{ title }}}`, [{ title: "Blog 3" }]));
...
test("Deep querying 5", async () => await runIt(`{allBlogs(comments: {text: "Comment 6", upVotes: 4 }){Blogs{ title }}}`, [{ title: "Blog 4" }]));
