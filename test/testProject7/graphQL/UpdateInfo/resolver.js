...
export default {
  ...
  Mutation: {
    async createUpdateInfo(root, args, context, ast) {
      ...
      if (await processHook(hooksObj, "UpdateInfo", "beforeInsert", newObject, root, args, context, ast) === false){
    ...
    async deleteUpdateInfo(root, args, context, ast) {
      ...
      let res = await processHook(hooksObj, "UpdateInfo", "beforeDelete", $match, root, args, context, ast);
      if (res === false){
        return false;
      }
