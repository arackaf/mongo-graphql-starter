...
export default {
  ...
  Mutation: {
    ...
    async updateDeleteInfo(root, args, context, ast) {
      ...
      let res = await processHook(hooksObj, "DeleteInfo", "beforeUpdate", $match, updates, root, args, context, ast);
      if (res === false){
        return { DeleteInfo: null };
      }
    ...
    async deleteDeleteInfo(root, args, context, ast) {
      ...
      let res = await processHook(hooksObj, "DeleteInfo", "beforeDelete", $match, root, args, context, ast);
      if (res === false){
        return false;
      }
