...
beforeAll(async () => {
  ...
  await Promise.all([dev, military, reading].map(subject => db.collection("keywords").insert(subject)));
...
test("Read authors with subjects", async () => {
  await queryAndMatchArray({
    query: `{allBooks(title_startsWith: "B"){Books{title, authors{name, subjects{name, keywords{keywordName}}}}}}`,
    coll: "allBooks",
    results: [
      { title: "Book 1", authors: [{ name: "Adam", subjects: [{ name: "JavaScript", keywords: [{ keywordName: "Development" }] }] }] },
      { title: "Book 2", authors: [{ name: "Adam", subjects: [{ name: "JavaScript", keywords: [{ keywordName: "Development" }] }] }] },
      { title: "Book 3", authors: [{ name: "Mallory", subjects: [{ name: "Literature", keywords: [{ keywordName: "Reading" }] }] }] }
    ]
  });
});
