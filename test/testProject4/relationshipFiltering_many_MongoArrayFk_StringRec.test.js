...
beforeAll(async () => {
  ...
  await db.collection("books").insertOne({ title: `Mallory Book 1`, pages: 100, mainAuthorId: mallory._idsS[0] });
  for (let i = 0; i < 100; i++) {
    for (let person of [adam, katie, laura]) {
      await db.collection("books").insertOne({ title: `${person.name} Book ${i + 1}`, pages: 100 + (i + 1), mainAuthorId: person._idsS[0] });
    }
  }
});
...
test("Read author's books_idsM_Main 1", async () => {
  await queryAndMatchArray({
    query: `{allAuthors(name_startsWith: "Adam"){Authors{name, books_idsM_Main(PAGE: 1, PAGE_SIZE: 3, FILTER: { pages_gt: 150 }, SORT: {pages: 1}){title}}}}`,
    coll: "allAuthors",
    results: [{ name: "Adam", books_idsM_Main: [{ title: "Adam Book 51" }, { title: "Adam Book 52" }, { title: "Adam Book 53" }] }]
  });
});

test("Read author's books_idsM_Main 2", async () => {
  await queryAndMatchArray({
    query: `{allAuthors(name_startsWith: "Adam"){Authors{name, books_idsM_Main(PAGE: 2, PAGE_SIZE: 3, FILTER: { pages_gt: 150 }, SORT: {pages: 1}){title}}}}`,
    coll: "allAuthors",
    results: [{ name: "Adam", books_idsM_Main: [{ title: "Adam Book 54" }, { title: "Adam Book 55" }, { title: "Adam Book 56" }] }]
  });
});
...
test("Read author's books_idsM_Main stand-alone", async () => {
  await queryAndMatchArray({
    ...
    coll: "allAuthors",
    results: [{ name: "Mallory", books_idsM_Main: [{ title: "Mallory Book 1" }] }]
  });
});